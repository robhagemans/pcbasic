...
class DataSegment(object):
    ...
    async def let_(self, args):
        """LET: assign value to variable or array."""
        name, indices = await anext(args)
        name = self.complete_name(name)
    ...
    async def varptr_str_(self, args):
        """VARPTR$: Get address of variable in string representation."""
        name = await anext(args)
        error.throw_if(not name, error.STX)
        indices = await anext(args)
        [_ async for _ in args]
        name = self.complete_name(name)
        if indices != []:
            # pre-allocate array elements, but not scalars which instead throw IFC if undefined
    ...
    async def swap_(self, args):
        """Swap two variables."""
        name1, index1 = await anext(args)
        name2, index2 = await anext(args)
    ...
    async def lset_(self, args):
        ...
        s = values.pass_string(await anext(args))
        [_ async for _ in args]
        self.set_variable(name, index, v.lset(s, justify_right=False))

    async def rset_(self, args):
    ...
    async def mid_(self, args):
        """MID$: set part of a string."""
        name, indices = await anext(args)
        name = self.complete_name(name)
        self._preallocate(name, indices)
        start = values.to_int(await anext(args))
        ...
        val = values.pass_string(await anext(args))
