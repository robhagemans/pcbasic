...
class StatementParser(object):
    ...
    def init_statements(self, session):
        ...
        self._callbacks = {
            ...
            tk.PAINT: partial(session.screen.drawing.paint_, input_methods=session.input_methods),
            tk.COM: session.basic_events.com_,
            tk.CIRCLE: session.screen.drawing.circle_,
            tk.DRAW: partial(session.screen.drawing.draw_, memory=session.memory, value_handler=session.values, input_methods=session.input_methods),
            ...
            tk.ON + tk.KEY: partial(session.basic_events.on_event_gosub_, session.program),
            tk.ON + '\xFE': partial(session.basic_events.on_event_gosub_, session.program),
            tk.ON + '\xFF': partial(session.basic_events.on_event_gosub_, session.program),
            ...
            tk.KEY: session.key_,
            tk.PUT + '(': partial(session.screen.drawing.put_, session.arrays, session.memory),
            tk.PUT: session.files.put_,
            tk.GET + '(': partial(session.screen.drawing.get_, session.arrays, session.memory),
