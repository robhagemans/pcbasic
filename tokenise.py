...
def tokenise_stream__old(ins, outs, one_line=False, onfile=True):
    
        
    while True:
        # skip whitespace at start of line
        d = skip_read(ins, tokenise_whitespace)
            
        
        
        if d in tokenise_endfile:
            return False
        elif d in tokenise_endline:
            if one_line:
                return True
            else:    
                continue
        ...
        while True: 
            ...
            # handle all end-of-line conditions    
            # handle CR/LF (FIXME: what happens with 0a0d?)     
            if d in tokenise_endline_nonnul:
                if d=='\x0d':
                    if ins.read(1) != '\x0A':
                        ins.seek(-1,1) 
                break
