...
#    'program':  { 'metavar':'basic_program', 'nargs':'?', 'help':'Input program file to run (default), load or convert.' },
    ...
    'dimensions-text':  { 'nargs':2, 'metavar':('X', 'Y'), 'help':'Set pixel dimensions for text mode. Default is 640,400. Graphical interface only.' },
    ...
    'strict-hidden-lines':{ 'action':'store_true', 'help':'Disable listing and ASCII saving of lines beyond 65530 (as in GW-BASIC). Use with care as this allows execution of unseen statements.' },
    'strict-protect':   { 'action':'store_true', 'help':'Disable listing and ASCII saving of protected files (as in GW-BASIC). Use with care as this allows execution of unseen statements.' },
    'caps':             { 'action':'store_true', 'help':'Start in CAPS LOCK mode.' },
    ...
    'strict-newline':   { 'action':'store_true', 'help':'Parse CR and LF strictly like GW-BASIC. May create problems with UNIX line endings.' },
    'pcjr-syntax':      { 'action':'store', 'nargs':1, 'choices':('pcjr', 'tandy'), 'help':'Enable PCjr/Tandy 1000 syntax extensions' },
    'pcjr-term':        { 'action':'store', 'metavar':'TERM.BAS', 'help':'Set the program run by the PCjr TERM command' },
    'video':            { 'action':'store', 'nargs':1, 'choices':('ega', 'cga', 'cga_old', 'pcjr', 'tandy'), 'help':'Set video capabilities' },
    'windows-map-drives':{ 'action':'store_true', 'help':'Map all Windows drive letters to PC-BASIC drive letters (Windows only)' },
    'cga-low':          { 'action':'store_true', 'help':'Use low-intensity palettes in CGA (for --video={cga,ega} only).' },
