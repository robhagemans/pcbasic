...
class CassetteTest(TestCase):
    ...
    async def test_cas_load(self):
        """Load from a CAS file."""
        with Session(devices={b'CAS1:': _input_file('test.cas')}) as s:
            await s.execute('load "cas1:test"')
            await s.execute('list')
    ...
    async def test_cas_save_load(self):
        ...
        with Session(devices={b'CAS1:': _output_file('test.cas')}) as s:
            await s.execute('save "cas1:empty"')
            await s.execute('load "cas1:test"')
            await s.execute('list')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[:3] == [
    ...
    async def test_cas_current_device(self):
        ...
        with Session(
            ...
            await s.execute('10 ?')
            await s.execute('save "Test"')
        with Session(
                devices={b'CAS1:': _output_file('test_current.cas')},
                current_device=b'CAS1:'
            ) as s:
            await s.execute('load "Test"')
            await s.execute('list')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[:2] == [
            b'Test    .B Found.',
    ...
    async def test_cas_text(self):
        ...
        with Session(devices={b'CAS1:': _output_file('test_prog.cas')}) as s:
            ...
            await s.execute('save "cas1:prog",A')
        with Session(devices={b'CAS1:': _output_file('test_prog.cas')}) as s:
            await s.execute('run "cas1:prog"')
            output = [_row.strip() for _row in self.get_text(s)]
            assert s.get_variable('A%') == 1234
            assert output[0] == b'prog    .A Found.'

    async def test_cas_data(self):
        ...
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute('open "cas1:data" for output as 1')
            await s.execute('print#1, 1234')
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute('open "cas1:data" for input as 1')
            await s.execute('input#1, A%')
            output = [_row.strip() for _row in self.get_text(s)]
            assert s.get_variable('A%') == 1234
            assert output[0] == b'data    .D Found.'

    async def test_wav_text(self):
    ...
    async def test_wav_save_load(self):
        ...
        with Session(devices={b'CAS1:': _output_file('test.wav')}) as s:
            await s.execute('10 A%=1234')
            await s.execute('save "cas1:prog"')
            await s.execute('20 A%=12345')
            await s.execute('save "cas1:Prog 2",A')
        with Session(devices={b'CAS1:': _output_file('test.wav')}) as s:
            # overwrite (part of) the first program
            await s.execute('save "cas1:"')
            # load whatever is next (this should be Prog 2)
            await s.execute('run "cas1:"')
            assert s.get_variable('A%') == 12345

    async def test_cas_empty(self):
    ...
    async def test_cas_already_open(self):
        ...
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute('open "cas1:data" for output as 1')
            await s.execute('open "cas1:data" for output as 2')
            output = [_row.strip() for _row in self.get_text(s)]
            assert output[0] == b'File already open\xff'

    async def test_cas_bad_name(self):
        ...
        try:
            ...
            pass
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute(b'open "cas1:\x02\x01" for output as 1')
            output = [_row.strip() for _row in self.get_text(s)]
            assert output[0] == b'Bad file number\xff'

    async def test_cas_bad_mode(self):
        ...
        try:
            ...
            pass
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute('open "cas1:test" for random as 1')
            output = [_row.strip() for _row in self.get_text(s)]
            assert output[0] == b'Bad file mode\xff'

    async def test_cas_bad_operation(self):
        ...
        try:
            ...
            pass
        with Session(devices={b'CAS1:': _output_file('test_data.cas')}) as s:
            await s.execute('open "cas1:test" for output as 1')
            await s.execute('? LOF(1)')
            await s.execute('? LOC(1)')
            output = [_row.strip() for _row in self.get_text(s)]
            assert output[:2] == [b'Illegal function call\xff', b'Illegal function call\xff']

    async def test_cas_no_name(self):
