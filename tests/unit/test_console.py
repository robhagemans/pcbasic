...
class ConsoleTest(TestCase):
    ...
    async def test_control_keys(self):
        """Test special keys in console."""
        with Session() as s:
            await s.execute(b'cls:print "%s"' % (_LIPSUM[:200],))
            # home
    ...
    async def test_control_keys_2(self):
        """Test special keys in console."""
        with Session() as s:
            # bel
            s.press_keys(u'1\a2')
    ...
    async def test_control_keys_3(self):
        """Test special keys in console."""
        with Session() as s:
            await s.execute(b'cls:print "%s"' % (_LIPSUM[:200],))
            # home
    ...
    async def test_control_home(self):
    ...
    async def test_control_printscreen(self):
    ...
    async def test_print_control(self):
        """Test printing control chars."""
        with Session() as s:
            await s.execute(b'print chr$(7) "1" chr$(9) "2" chr$(&h1c) "3" chr$(&h1d) "4"')
            await s.execute(b'print chr$(7)+"1"+chr$(9)+"2"+chr$(&h1c)+"3"+chr$(&h1d)+"4"')
        assert self.get_text_stripped(s) == [b'1       2 4', b'1       2 4'] + [b''] * 23

    async def test_print_control_2(self):
        """Test printing control chars."""
        with Session() as s:
            await s.execute(b'print "  1" chr$(&h1f) chr$(&h1f) "2" chr$(&h1e) "3" chr$(&h0b) "4"')
        assert self.get_text_stripped(s) == [b'4 1', b'    3', b'   2'] +[b''] * 22

    async def test_print_control_3(self):
        """Test printing control chars."""
        with Session() as s:
            await s.execute(b'print "  1" chr$(&h1f) chr$(&h1f) "2" chr$(&h1e) "3" chr$(&h0c) "4"')
        assert self.get_text_stripped(s) == [b'4'] + [b''] * 24

    async def test_input_wrapping_line(self):
        """Test input on top of an existing long line."""
        with Session() as s:
            s.press_keys(u'1\r')
            await s.execute(b'cls:print "%s"' % (_LIPSUM[:200],))
            await s.execute(b'locate 1,1: input a$')
    ...
    async def test_cursor_move(self):
        ...
        with Session() as s:
            await s.execute(b'locate 1,1: print"xy";: print csrlin; pos(0);:locate 10,1')
        # normal behaviour, cursor moves
        assert self.get_text_stripped(s) == [b'xy 1  6'] + [b'']*24, repr(s.get_text())

    async def test_cursor_overflow(self):
        """Test cursor movement after print on last column."""
        with Session() as s:
            await s.execute(b'locate 1,80: print"x";')
    ...
    async def test_cursor_overflow_cr(self):
        """Test cursor movement after print char and cr on last column."""
        with Session() as s:
            await s.execute(b'locate 1,80: print"x";chr$(13);')
    ...
    async def test_cursor_overflow_char(self):
        """Test cursor movement after print two chars on last column."""
        with Session() as s:
            await s.execute(b'locate 1,80: print"x" "y";')
    ...
    async def test_cursor_overflow_word(self):
        """Test cursor movement after print a two-char word on last column."""
        with Session() as s:
            await s.execute(b'locate 1,80: print"xy";')
        # cursor has moved after printing char
        assert s._impl.text_screen.current_row == 2, s._impl.text_screen.current_row
    ...
    async def test_cursor_overflow_cr_char(self):
        """Test cursor movement after print char, return, char on last column."""
        with Session() as s:
            await s.execute(b'locate 1,80: print"x" chr$(13) "y";')
    ...
    async def test_cursor_bottom(self):
        """Test cursor movement after print on last column, last row."""
        with Session() as s:
            # normal behaviour
            await s.execute(b'locate 24,80: print"x";')
    ...
    async def test_cursor_bottom_cr(self):
        """Test cursor movement after print two chars on last column, last row."""
        with Session() as s:
            await s.execute(b'locate 24,80: print"x";chr$(13);')
    ...
    async def test_cursor_bottom_char(self):
        """Test cursor movement after print char and return on last column, last row."""
        with Session() as s:
            await s.execute(b'locate 24,80: print"x" "y";')
    ...
    async def test_cursor_bottom_cr_char(self):
        """Test cursor movement after print char, return, char on last column, last row."""
        with Session() as s:
            await s.execute(b'locate 24,80: print"x" chr$(13) "y";')
        # cursor has moved after printing char, but no extra line for CR
        assert s._impl.text_screen.current_row == 24, s._impl.text_screen.current_row
        assert s._impl.text_screen.current_col == 2, s._impl.text_screen.current_col
        assert self.get_text_stripped(s) == [b''] * 22 + [b' '*79 + b'x', b'y', b''], repr(self.get_text_stripped(s))
