...
class DosTest(TestCase):
    ...
    async def test_shell(self):
        ...
        with Session(shell=pythoncall(helper), codepage=read_codepage('850')) as s:
            ...
            await s.execute(u'SHELL "echo 1"')
            # test non-ascii char
            await s.execute(u'SHELL "echo £"')
            # outputs come through stderr
            await s.execute(u'SHELL "x"')
            # test non-ascii char
            await s.execute(u'SHELL "£"')
    ...
    @unittest.skipIf(PY2, 'shell codepage agreement known not to work in Python 2.')
    async def test_shell_utf16(self):
    ...
    @unittest.skipIf(PY2, 'shell codepage agreement known not to work in Python 2.')
    async def test_interactive_shell_no_lf_at_end(self):
    ...
    async def test_environ(self):
        ...
        with Session() as s:
            await s.execute(u'ENVIRON "test=ok"')
            assert await s.evaluate(u'ENVIRON$("test")') == b'ok'
            assert await s.evaluate(u'ENVIRON$("TEST")') == b'ok'
            assert await s.evaluate(u'ENVIRON$("Test")') == b'ok'
            await s.execute(u'ENVIRON "TEST=OK"')
            assert await s.evaluate(u'ENVIRON$("test")') == b'OK'
    ...
    async def test_environ_noascii_key(self):
        """Test ENVIRON statement with non-ascii key."""
        with Session() as s:
            await s.execute(u'ENVIRON "t£st=ok"')
            assert self.get_text_stripped(s)[0] == b'Illegal function call\xff'

    async def test_environ_fn_noascii_key(self):
        """Test ENVIRON$ function with non-ascii key."""
        with Session() as s:
            await s.evaluate(u'ENVIRON$("t£st")')
            assert self.get_text_stripped(s)[0] == b'Illegal function call\xff'

    async def test_environ_noascii_value(self):
        """Test ENVIRON statement with non-ascii values."""
        with Session() as s:
            await s.execute(u'ENVIRON "TEST=£"')
