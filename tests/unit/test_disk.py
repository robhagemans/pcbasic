...
class DiskTest(TestCase):
    ...
    async def test_text(self):
        ...
        with Session(devices={b'A': self.output_path()}, current_device='A:') as s:
            await s.execute('10 A%=1234')
            await s.execute('save "prog",A')
        with Session(devices={b'A': self.output_path()}, current_device='A:') as s:
    ...
    async def test_files(self):
        ...
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('save "A:prog",A')
            await s.execute('files "A:"')
            await s.execute('print "##"')
    ...
    async def test_files_longname(self):
    ...
    async def test_files_wildcard(self):
        ...
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('files "A:aa?.txt"')
            output = [_row.strip() for _row in self.get_text(s)]
        assert b'AAA     .TXT' in output[1]
        assert b'AAB     .TXT' in output[1]
        # no match
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('files "A:b*.txt"')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[1] == b'File not found\xff'

    async def test_internal_disk_files(self):
    ...
    async def test_internal_disk_unbound_files(self):
    ...
    async def test_disk_data(self):
        """Write and read data to a text file."""
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:data" for output as 1')
            await s.execute('print#1, 1234')
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:data" for input as 1')
            await s.execute('input#1, A%')
            assert s.get_variable('A%') == 1234

    async def test_disk_data_utf8(self):
    ...
    async def test_disk_data_lf(self):
        ...
        with Session(devices={b'A': self.output_path()}, soft_linefeed=True) as s:
            await s.execute('open "a:data" for input as 1')
            await s.execute('line input#1, a$')
            await s.execute('line input#1, b$')
            await s.execute('line input#1, c$')
    ...
    async def test_disk_data_append(self):
    ...
    async def test_disk_random(self):
        """Write and read data to a random access file."""
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:data" for random as 1')
            await s.execute('field#1, 20 as a$, 20 as b$')
            await s.execute('lset b$="abcde"')
            await s.execute('print#1, 1234')
            await s.execute('put#1, 1')
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:data" for random as 1')
            await s.execute('field#1, 20 as a$, 20 as b$')
            await s.execute('get#1, 1')
            assert s.get_variable('A$') == b' 1234 \r\n'.ljust(20, b'\0')
            assert s.get_variable('B$') == b'abcde'.ljust(20, b' ')

    async def test_match_name(self):
        ...
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:mixcase.txt" for output as 1')
            await s.execute('print#1, 1234')
        with Session(devices={b'A': self.output_path()}) as s:
            await s.execute('open "a:MIXCASE.TXT" for input as 1')
            await s.execute('input#1, A%')
            assert s.get_variable('A%') == 1234
    ...
    async def test_match_name_non_ascii(self):
    ...
    async def test_path(self):
        ...
        with Session(devices={b'A': self.output_path()}, current_device='A:') as s:
            ...
            await s.execute('open "a\\b\\rel" for output as 1:close')
            # convoluted path
            await s.execute('open "a\\b\\..\\..\\a\\.\\dots" for output as 1:close')
            # set cwd
            await s.execute('chdir "a"')
            # absolute path
            await s.execute('open "\\a\\b\\abs" for output as 1:close')
            # relative path from cwd
            await s.execute('open ".\\this" for output as 1:close')
            await s.execute('open "..\\parent" for output as 1:close')
    ...
    async def test_directory_ops(self):
        """Test directory operations."""
        with Session(devices={b'A': self.output_path()}, current_device='A:') as s:
            await s.execute('mkdir "test"')
            await s.execute('mkdir "test\\test2"')
            await s.execute('chdir "test"')
            await s.execute('mkdir "remove"')
            await s.execute('rmdir "remove"')
    ...
    async def test_file_ops(self):
        ...
        with Session(
            ...
            await s.execute('name "testfile" as "newname"')
            # rename across disks
            await s.execute('name "newname" as "b:fail"')
            # file already exists
            await s.execute('name "newname" as "delete.txt"')
            await s.execute('kill "delete.txt"')
            await s.execute('kill "delete?.txt"')
            await s.execute('kill "delete*"')
            # file not found
            await s.execute('kill "notfound"')
            # file not found
            await s.execute('kill "not*.*"')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[:4] == [
    ...
    async def test_files_cwd(self):
        """Test directory listing, not on root."""
        os.mkdir(self.output_path('a'))
        with Session(devices={b'A': self.output_path()}, current_device='A:') as s:
            await s.interact('chdir "a"')
            await s.interact('files')
            await s.interact('files ".."')
            await s.interact('files "..\\"')
            output = [_row.strip() for _row in self.get_text(s)]
    ...
    async def test_files_no_disk(self):
        """Test directory listing, non-existing device."""
        with Session() as s:
            await s.interact('files "A:"')
            output = [_row.strip() for _row in self.get_text(s)]
    ...
    async def test_close_not_open(self):
        ...
        with Session() as s:
            await s.interact('close#2')
            output = [_row.strip() for _row in self.get_text(s)]
        # no error
        assert output[0] == b''

    async def test_mount_dict_spec(self):
        """Test mount dict specification."""
        with Session(devices={b'A': self.output_path()}) as s:
            await s.interact('open "A:test" for output as 1: print#1, 42: close 1')
        # lowercase
        with Session(devices={b'a': self.output_path()}) as s:
            await s.interact('open "A:test" for input as 1: input#1, A%')
    ...
    async def test_open_bad_device(self):
    ...
    async def test_open_null_device(self):
        """Test the NUL device."""
        with Session(devices={b'A': self.output_path()}) as s:
            await s.interact('open "NUL" for output as 1: print#1, 42: close 1')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[0] == b''

    async def test_open_bad_number(self):
        """Test opening to a bad file number."""
        with Session(devices={b'A': self.output_path()}, current_device='A') as s:
            await s.interact('open "TEST" for output as 4')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[0] == b'Bad file number\xff'

    async def test_open_reuse_number(self):
        """Test opening to a number taht's already in use."""
        with Session(devices={b'A': self.output_path()}, current_device='A') as s:
            await s.interact('open "TEST" for output as 1')
            await s.interact('open "TEST2" for output as 1')
            output = [_row.strip() for _row in self.get_text(s)]
        assert output[0] == b'File already open\xff'

    async def test_long_filename(self):
        ...
        with Session(devices={b'A': self.output_path()}) as s:
            ...
            for name, found in basicnames.items():
                await s.interact(b'run "a:%s"' % (name,))
    ...
    async def test_kill_long_filename(self):
        ...
        with Session(devices={b'A': self.output_path()}) as s:
            await s.interact('kill "VERYLONG.EXT"')
