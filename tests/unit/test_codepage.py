...
class CodepageTest(TestCase):
    ...
    async def test_nobox(self):
        ...
        with Session(
            ...
            await s.execute('open "c:boxtest.txt" for output as 1')
            await s.execute('PRINT#1, CHR$(218);STRING$(10,CHR$(196));CHR$(191)')
            # to screen
            await s.execute('PRINT CHR$(218);STRING$(10,CHR$(196));CHR$(191)')
            # bytes text
            # bytes text
    ...
    async def test_box2(self):
        """Test box protection cases."""
        cp_936 = read_codepage('936')
        with Session(codepage=cp_936, box_protect=True) as s:
            await s.execute('a$= "+"+STRING$(3,CHR$(196))+"+"')
            await s.execute('b$= "+"+STRING$(2,CHR$(196))+"+"')
            await s.execute('c$= "+"+STRING$(1,CHR$(196))+"+"')
            await s.execute('d$= "+"+CHR$(196)+chr$(196)+chr$(190)+chr$(196)+"+"')
    ...
    async def test_hello(self):
        ...
        for cp, hi in hello.items():
            ...
            with Session(
                ...
                await s.execute(u'cls:print "{}"'.format(hi))
                ...
                await s.execute(u'open "c:{}" for input as 1'.format(hi))
    ...
    async def test_grapheme_sequence(self):
