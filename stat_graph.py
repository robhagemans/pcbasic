...
def exec_pset(ins, default_colour=-1):
    ...
    relative = skip_white_read_if(ins, '\xCF') # STEP
    x,y = parse_coord(ins)
...
def exec_line_graph(ins):
    ...
    if skip_white_read_if(ins, ','):
        ...
        if skip_white_read_if(ins, ','):
            if skip_white_read_if(ins, 'B'):
                mode='B'
                if skip_white_read_if(ins, 'F'):         
                    mode='BF'
            if skip_white_read_if(ins, ','):
...
def exec_view_graph(ins):
    if not glob.scrn.graphics_mode:
        raise error.RunError(5)
    absolute = skip_white_read_if(ins, '\xC8') #SCREEN
...
def exec_window(ins):
    if not glob.scrn.graphics_mode:
        raise error.RunError(5)
    cartesian = not skip_white_read_if(ins, '\xC8') #SCREEN
...
def draw_parse_gml(gml):
    ...
    while True:
        c = skip_read(gmls, gml_whitespace).upper()
        ...
        # two-variable movement command
            ...
            relative =  skip(gmls,gml_whitespace) in ('+','-')
            x = draw_parse_number(gmls)
            
            if skip(gmls, gml_whitespace) !=',':
                raise error.RunError(5)
            else:
                gmls.read(1)
...
def play_parse_mml(mml):
    ...
    while True:
        ...
        if c=='':
            ...
            dur=play_length
            while True:    
                c = skip(gmls, gml_whitespace).upper()
                if c=='.':
                    gmls.read(1)
                    dur *= 1.5
